...
# I Ciclo 2024
#
# Problema 2. Escriba en lenguaje ensamblador de Mips el siguiente código que está en alto nivel:
#													i=0; 
#													while(A[2*i]!=0) 
#													{ 
# 														A[i]=A[2*i-1]+A[2*i+1]; 
...
#													}
#
# La solucion especificamente se ve en la funcion "Problema2", lo demas es para imprimir y poder demostrar los resultados
##################################################################################################################################################

.data
	# Arreglos:
	A: .word 4, 5, -2, 8, 9, 10, 3, 14, 0, 12,
...
.text
	main:	
		...
		jal Problema2 			     		# Salta a la funcion Problema1		
	...
	Problema2:
		...
		while:	
			sll $t7, $t0, 1             		# $t7 = 2*i
			sll $t7, $t7, 2             		# $t7 = (2*i)*4
			add $t7, $t7, $s0	     		# $t7 = A + (2*i)*4 Direccion de A[2*i]
			lw $t5, 0($t7)				# $t5 = A[2*i]
			
			beq $t5, $0, endwhile			# Si A[2*i] = 0 salta a endwhile
			
			lw $t4, -4($t7)				# $t4 = A[2*i-1]
			lw $t6, 4($t7)				# $t6 = A[2*i+1]
			add $t5, $t4, $t6			# $t5 = A[2*i-1]+A[2*i+1]
